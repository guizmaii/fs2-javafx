...
package object jfx {
  ...
  @inline private def liftGen[A, C1, C2, S](c1: C1, consInit: Boolean)(bind: (C1 => Unit) => S, unbind: (C1, S) => Unit)(
    f: C1 => C2
  ...
  def liftSetRaw[A, C](prop: ObservableSet[A], consInit: Boolean)(f: ObservableSet[A] => C)(implicit cs: ContextShift[IO]): Stream[IO, C] =
    liftGen[A, ObservableSet[A], C, SetChangeListener[A]](prop, consInit)(
      f => {
  ...
  def liftSet[A, C](prop: ObservableSet[A], consInit: Boolean)(factory: Factory[A, C])(implicit cs: ContextShift[IO]): Stream[IO, C] =
    liftSetRaw[A, C](prop, consInit) { xs =>
      import scala.jdk.CollectionConverters._
      xs.asScala.to(factory)
    }

  def liftListRaw[A, C](prop: ObservableList[A], consInit: Boolean)(
    f: ObservableList[A] => C
  )(implicit cs: ContextShift[IO]): Stream[IO, C] =
    liftGen[A, ObservableList[A], C, ListChangeListener[A]](prop, consInit)(
      f => {
  ...
  def liftList[A, C](prop: ObservableList[A], consInit: Boolean)(factory: Factory[A, C])(implicit cs: ContextShift[IO]): Stream[IO, C] =
  ...
  @inline private def lift0[B, A](
    prop: ObservableValue[B],
    consInit: Boolean
  )(implicit cs: ContextShift[IO], ev: B => A): Stream[IO, Option[A]] =
    liftGen[B, ObservableValue[B], Option[A], InvalidationListener](prop, consInit)(bindObservableValue(prop), unbindObservableValue)(v =>
      Option(v.getValue).map(ev)
    )
  ...
  @inline private def lift1[O <: ObservableValue[P], P, B, A](prop: O, getter: O => B, consInit: Boolean)(
    ...
    liftGen[B, O, Option[A], InvalidationListener](prop, consInit)(bindObservableValue(prop), unbindObservableValue)(v =>
      Option(getter(v)).map(ev)
    )

  def lift[A <: Any](prop: ObservableValue[A], consInit: Boolean)(implicit cs: ContextShift[IO]): Stream[IO, Option[A]] =
    lift0[A, A](prop, consInit)

  def lift(prop: ObservableBooleanValue, consInit: Boolean)(implicit cs: ContextShift[IO]): Stream[IO, Option[Boolean]] =
    lift1[ObservableBooleanValue, java.lang.Boolean, java.lang.Boolean, Boolean](prop, _.get, consInit)

  def lift(prop: ObservableDoubleValue, consInit: Boolean)(implicit cs: ContextShift[IO]): Stream[IO, Option[Double]] =
    lift1[ObservableDoubleValue, Number, java.lang.Double, Double](prop, _.get, consInit)

  def lift(prop: ObservableFloatValue, consInit: Boolean)(implicit cs: ContextShift[IO]): Stream[IO, Option[Float]] =
    lift1[ObservableFloatValue, Number, java.lang.Float, Float](prop, _.get, consInit)

  def lift(prop: ObservableIntegerValue, consInit: Boolean)(implicit cs: ContextShift[IO]): Stream[IO, Option[Int]] =
    lift1[ObservableIntegerValue, Number, java.lang.Integer, Int](prop, _.get, consInit)

  def lift(prop: ObservableLongValue, consInit: Boolean)(implicit cs: ContextShift[IO]): Stream[IO, Option[Long]] =
    lift1[ObservableLongValue, Number, java.lang.Long, Long](prop, _.get, consInit)

  def handleEvent[A <: FXEvent, B](
    prop: ObjectProperty[_ >: EventHandler[A]]
  )(f: A => Option[B])(implicit cs: ContextShift[IO]): Stream[IO, B] =
    for {
  ...
  def handleEvent[A <: FXEvent, B](
    prop: Node
  )(eventType: EventType[A], filter: Boolean = false)(f: A => Option[B])(implicit cs: ContextShift[IO]): Stream[IO, B] =
    for {
      q <- Stream.eval(Queue.bounded[IO, B](maxSize = 1))
      _ <- Stream.bracket(IO {
  ...
  def nodeCellFactory[A, N](prop: ObjectProperty[Callback[ListView[A], ListCell[A]]])(mkNode: () => N)(
    toNode: N => Node
  )(unsafeUpdate: (Option[A], N) => IO[Unit])(implicit fxcs: FXContextShift, cs: ContextShift[IO]): Stream[IO, (Option[A], N)] = for {
