...
object syntax {
  ...
  implicit class ObservableListInstances[A](private val xs: ObservableList[A]) extends AnyVal {
    def observeRaw[C](consInit: Boolean)(f: ObservableList[A] => C)(implicit cs: ContextShift[IO]): Stream[IO, C] =
      liftListRaw(xs, consInit)(f)
    def observe[C](consInit: Boolean)(factory: Factory[A, C])(implicit cs: ContextShift[IO]): Stream[IO, C] =
      liftList(xs, consInit)(factory)
  }

  implicit class ObservableSetInstances[A](private val xs: ObservableSet[A]) extends AnyVal {
    def observeRaw[C](consInit: Boolean)(f: ObservableSet[A] => C)(implicit cs: ContextShift[IO]): Stream[IO, C] =
      liftSetRaw(xs, consInit)(f)
