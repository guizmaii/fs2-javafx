...
trait FXApp extends IOApp {
  ...
  override final def run(args: List[String]): IO[ExitCode] = (for {
    ...
    _ <- Stream.eval(IO(c(halt, fxContextShift)).flatMap { case (ctx, stage) => runFX(args, ctx, stage).interruptWhen(halt).compile.drain })
...
object FXApp {
   ...
   */
  class FXContext(private val app: Application, private val halt: Signal[IO, Boolean])(implicit val fxcs: FXContextShift) {
  ...
  private var ctx: Either[Throwable, (SignallingRef[IO, Boolean], FXContextShift) => (FXContext, Stage)] => Unit = _
